...
// 
//
inline Tile_Chunk_Position getChunkPositionFor(Tile_Map* tile_map, u32 abs_tile_x, u32 abs_tile_y, u32 abs_tile_z) {
...
inline u32 getTileValue(Tile_Map* tile_map, Tile_Chunk* tile_chunk, u32 test_tile_x, u32 test_tile_y, u32 test_tile_z) {
    u32 result = 0;
...
static u32 getTileValue(Tile_Map* tile_map, u32 abs_tile_x, u32 abs_tile_y, u32 abs_tile_z) {
    //bool32 empty = false;

    Tile_Chunk_Position chunk_pos = getChunkPositionFor(tile_map, abs_tile_x, abs_tile_y, abs_tile_z);
    Tile_Chunk* tile_chunk = getTileChunk(tile_map, chunk_pos.tile_chunk_x, chunk_pos.tile_chunk_y, chunk_pos.tile_chunk_z);
    u32 tile_chunk_value = getTileValue(tile_map, tile_chunk, chunk_pos.rel_tile_x, chunk_pos.rel_tile_y, abs_tile_z);

    return tile_chunk_value;
}

inline u32 getTileValue(Tile_Map* tile_map, Tile_Map_Position test_pos) {
    u32 tile_chunk_value = getTileValue(tile_map, test_pos.abs_tile_x, test_pos.abs_tile_y, test_pos.abs_tile_z);
...
inline Tile_Map_Difference tileSubtract(Tile_Map* tile_map, Tile_Map_Position* a, Tile_Map_Position* b) {
    ...
    result.dXY = tile_map->tile_side_in_meters * diff_tile_xy + (a->offset - b->offset);
    result.dz = tile_map->tile_side_in_meters * diff_tile_z;

    return result;
}

inline Tile_Map_Position centeredTilePoint(u32 abs_tile_x, u32 abs_tile_y, u32 abs_tile_z) {
...
inline Tile_Map_Position offsetTilePosition(Tile_Map* tile_map, Tile_Map_Position p, v2 offset) {
    p.offset += offset;
