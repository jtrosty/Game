...
inline Entity entityFromHighIndex(Game_State *game_state,
...
inline High_Entity *makeEntityHighFrequency(Game_State *game_state,
                                            Low_Entity *entity_low,
                                            u32 low_index, v2 camera_space_p) {
  ...
  if (high_index) {
    ...
    if (high_index != last_high_index) {
      ...
      *del_entity = *last_entity;
      game_state->low_entities[last_entity->low_entity_index]
          .high_entity_index = high_index;
    }
...
inline bool32 validateEntityPairs(Game_State *game_state) {
  ...
  for (u32 high_entity_index = 1;
       high_entity_index < game_state->high_entity_count; ++high_entity_index) {
...
inline void offsetAndCheckFrequencyByArea(Game_State *game_state, v2 offset,
                                          Rectangle2 high_frequency_bounds) {
...
static void setCamera(Game_State *game_state, World_Position new_camera_p) {
  ...
  World_Difference d_camera_p =
      worldSubtract(world, &new_camera_p, &game_state->camera_pos);
  ...
  offsetAndCheckFrequencyByArea(game_state, entity_offset_for_frame,
                                camera_bounds);

  World_Position min_chunk_p =
      mapIntoChunkSpace(world, new_camera_p, getMinCorner(camera_bounds));
  World_Position max_chunk_p =
  ...
  for (i32 chunk_y = min_chunk_p.chunk_y; chunk_y <= max_chunk_p.chunk_y;
    ...
    for (i32 chunk_x = min_chunk_p.chunk_x; chunk_x <= max_chunk_p.chunk_x;
      ...
      World_Chunk *chunk =
          getWorldChunk(world, chunk_x, chunk_y, new_camera_p.chunk_z);
      if (chunk) {
          ...
          for (u32 entity_index_index = 0;
            ...
            if (low->high_entity_index == 0) {
              ...
              if (isInRectangle(camera_bounds, camera_space_p)) {
                makeEntityHighFrequency(game_state, low, low_entity_index,
                                        camera_space_p);
              }
            }
          }
  ...
  to make high freq i32 min_tile_x = new_camera_p.abs_tile_x - (tile_span_x /
...
internal void moveEntity(Game_State *game_state, Entity entity, real32 dt,
                         v2 ddp) {
  ...
  // p' = 1/2 a t^32 + vt + p;
  v2 player_delta = (0.5f * ddp * math_square(dt) + entity.high->dp * dt);
  entity.high->dp = ddp * dt + entity.high->dp;
  v2 new_player_pos = old_player_pos + player_delta;

  // TODO: ODE here?
  /*
  u32 min_tile_x = Minimum(old_player_pos.abs_tile_x,
  new_player_pos.abs_tile_x); u32 min_tile_y =
  Minimum(old_player_pos.abs_tile_y, new_player_pos.abs_tile_y); u32 max_tile_x
  = Maximum(old_player_pos.abs_tile_x, new_player_pos.abs_tile_x); u32
  max_tile_y = Maximum(old_player_pos.abs_tile_y, new_player_pos.abs_tile_y);

  u32 entity_tile_width = ceilReal32ToInt32(entity->width /
  tile_map->tile_side_in_meters); u32 entity_tile_height =
  ceilReal32ToInt32(entity->heigth / tile_map->tile_side_in_meters);
  ...
  */
  ...
  for (u32 iteration = 0; (iteration < 4); ++iteration) {
    ...
    if (hit_high_entity_index) {
      entity.high->dp =
          entity.high->dp -
          1 * math_inner(entity.high->dp, wall_normal) * wall_normal;
      ...
      Low_Entity *hit_low =
          game_state->low_entities + hit_high->low_entity_index;
  ...
  } else if (absoluteValue(entity.high->dp.x) >
             absoluteValue(entity.high->dp.y)) {
  ...
  World_Position new_p = mapIntoChunkSpace(
      game_state->world, game_state->camera_pos, entity.high->p);
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender) {
  ...
  if (!memory->is_initialized) {
    ...
    Debug_Read_File_Result file =
        memory->DEBUG_platformReadEntireFile(thread, filename);
    if (file.contents) {
  ...
  for (int controller_index = 0;
    ...
    if (low_index == 0) {
      if (controller->start.ended_down) {
        ...
        game_state->player_index_for_controllers[controller_index] =
            player_entity.low_index;
      }
    } else {
      ...
      if (controller->is_analog) {
        ddp = v2{controller->left_stick_average_x,
  ...
  Entity camera_following_entity = forceEntityIntoHigh(
      game_state, game_state->camera_following_entity_index);

  if (camera_following_entity.high) {
    // game_state->camera_pos.abs_tile_z =
    // camera_following_entity.low->p.abs_tile_z;
  ...
  for (u32 high_entity_index = 1;
    ...
    Low_Entity *low_entity =
        game_state->low_entities + high_entity->low_entity_index;
    ...
    real32 ddZ = -9.8f;
    high_entity->z =
        0.5f * ddZ * math_square(dt) + high_entity->dZ * dt + high_entity->z;
    ...
    real32 player_ground_point_x =
        screen_center_x + (meters_to_pixels * high_entity->p.x);
    real32 player_ground_point_y =
        screen_center_y - (meters_to_pixels * high_entity->p.y);
    ...
    if (low_entity->type == entityType_Hero) {
      ...
      drawRectangle(buffer, player_left_top,
                    player_left_top + (entity_width_height * meters_to_pixels),
                    player_red, player_green, player_blue);
      // drawBitmap(buffer, &hero_bitmaps->torso, player_ground_point_x,
      // player_ground_point_y, hero_bitmaps->align_x, hero_bitmaps->align_y);
      // drawBitmap(buffer, &hero_bitmaps->cape, player_ground_point_x,
      // player_ground_point_y, hero_bitmaps->align_x, hero_bitmaps->align_y);
      // drawBitmap(buffer, &hero_bitmaps->head, player_ground_point_x,
      // player_ground_point_y, hero_bitmaps->align_x, hero_bitmaps->align_y);
      // Hero_Bitmaps* hero_bitmaps =
      // &game_state->hero_bitmaps[entity_ptr->facingDirection];
    } else {
      drawRectangle(buffer, player_left_top,
  ...
  drawBitmap(buffer, &game_state->DEBUG_bitmap, screen_center_x,
             screen_center_y, 0, 1.0f);
