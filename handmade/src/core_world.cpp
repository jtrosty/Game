...
inline bool32 isCanonical(World* world, v2 offset) {
    bool32 result = isCanonical(world, offset.x) &&
...
inline bool32 areInSameChunk(World* world, World_Position* a, World_Position* b) {
    ...
    bool32 result = ((a->chunk_x == b->chunk_x) && 
                     (a->chunk_y == b->chunk_y) && 
                     (a->chunk_z == b->chunk_z));
...
inline World_Chunk* getWorldChunk(World* world, i32 chunk_x, i32 chunk_y, i32 chunk_z,
...
static void initializeWorld(World* world, real32 tile_side_in_meters) {
    ...
    for(u32 chunk_index = 0; chunk_index < ArrayCount(world->chunk_hash); ++chunk_index) {
        world->chunk_hash[chunk_index].chunk_x = TILE_CHUNK_UNINITIALIZED;
        world->chunk_hash[chunk_index].first_block.entity_count = 0;
    }
}

inline void recanonicalizeCoordinate(World* world, i32* tile, real32* tile_rel) {
    ...
    //
    i32 offset = roundReal32ToInt32(*tile_rel / world->chunk_side_in_meters); // this will be 1 or -1
...
static World_Position mapIntoChunkSpace(World* world, World_Position base_pos, v2 offset) {
...
inline World_Position chunkPositionFromTilePosition(World* world, i32 abs_tile_x, i32 abs_tile_y, i32 abs_tile_z) {
    World_Position result = {};
    ...
    result.offset.x = (real32)((abs_tile_x - TILES_PER_CHUNK / 2) - (result.chunk_x * TILES_PER_CHUNK)) * world->tile_side_in_meters;
    result.offset.y = (real32)((abs_tile_y - TILES_PER_CHUNK / 2) - (result.chunk_y * TILES_PER_CHUNK)) * world->tile_side_in_meters;
...
inline World_Difference worldSubtract(World* world, World_Position* a, World_Position* b) {
    ...
    result.dXY = world->tile_side_in_meters * diff_tile_xy + (a->offset - b->offset);
    result.dz = world->tile_side_in_meters * diff_tile_z;

    return result;
}

inline World_Position centeredChunkPoint(i32 chunk_x, i32 chunk_y, i32 chunk_z) {
    World_Position result = {};
...
inline World_Position centeredChunkPoint(u32 chunk_x, u32 chunk_y, u32 chunk_z) {
    World_Position result = {};
...
inline void changeEntityLocationRaw(Memory_Arena* arena, World* world, u32 low_entity_index, 
    ...
    if (old_p && new_p && areInSameChunk(world, old_p, new_p)) {
        ...
        if(old_p) {
            ...
            World_Chunk* chunk = getWorldChunk(world, new_p->chunk_x, new_p->chunk_y, new_p->chunk_z, arena);
            Assert(chunk);
