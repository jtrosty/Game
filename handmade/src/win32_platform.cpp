...
/*
  ...
  - Make the right calls so Windows doesn't think we're "still loading" for a
  bit after we actually start
...
global_variable WINDOWPLACEMENT global_window_position = {
    sizeof(global_window_position)};

static LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                   LPARAM lParam);
static void
win32ProcessPendingMessage(Game_Controller_Input* keyboard_controller);
static void win32LoadXInput(void);
static void win32_process_x_input_button(DWORD x_input_state,
                                         Game_Button_State* old_state,
                                         DWORD button_bit,
                                         Game_Button_State* new_state);
static real32 win32_process_x_input_stick(SHORT value,
                                          SHORT dead_zone_threshold);
static void win32_process_keyboard_message(Game_Button_State* new_state,
                                           bool32 is_down);
...
// Xinput Get State
// ///////////////////////////////////////////////////////////// This makes name
// represetn the funciton address on the right
#define X_INPUT_GET_STATE(name)                                                \
  DWORD WINAPI name(DWORD dw_user_index, XINPUT_STATE* p_state)
// This then
...
#define X_INPUT_SET_STATE(name)                                                \
  DWORD WINAPI name(DWORD dw_user_index, XINPUT_VIBRATION* p_vibration)
...
static real32 win32_process_x_input_stick(SHORT value,
  ...
  if (value < -dead_zone_threshold) {
    result = (real32)((value + dead_zone_threshold) /
...
static Win32_Game_Code win32_loadGameCode(char* source_dll_name,
                                          char* temp_dll_name) {
  ...
  if (result.game_code_dll) {
    result.update_and_render_fn = (game_update_and_render*)GetProcAddress(
        result.game_code_dll, "GameUpdateAndRender");
    result.get_sound_samples_fn = (game_get_sound_samples*)GetProcAddress(
        result.game_code_dll, "GameGetSoundSamples");

    result.is_valid =
        (result.update_and_render_fn && result.update_and_render_fn);
  }
...
static void win32_buildEXEPathFileName(Win32_State* state, char* filename,
                                       int dest_count, char* dest) {
  catStrings(state->one_past_last_EXE_filename_slash - state->EXE_file_name,
             state->EXE_file_name, stringLength(filename), filename, dest_count,
             dest);
}

static void win32_getEXEFileName(Win32_State* state) {
  DWORD size_of_filename =
      GetModuleFileNameA(0, state->EXE_file_name, sizeof(state->EXE_file_name));
...
//*********************************//
static void win32_init_direct_sound(HWND window, i32 samples_per_second,
                                    i32 buffer_size) {
  HMODULE d_sound_library = LoadLibraryA("dsound.dll");
  if (d_sound_library) {

    _DirectSoundCreate* directSoundCreate = (_DirectSoundCreate*)GetProcAddress(
        d_sound_library, "DirectSoundCreate");
    ...
    if (debug_create_dsound == DS_OK) {
      ...
      pri_wave_format.nSamplesPerSec = 50000;
      pri_wave_format.wBitsPerSample = 16;
      pri_wave_format.nBlockAlign =
          (pri_wave_format.wBitsPerSample * pri_wave_format.nChannels) / 8;
      pri_wave_format.nAvgBytesPerSec =
          pri_wave_format.nSamplesPerSec * pri_wave_format.nBlockAlign;
      ...
      pri_buffer_descriptipn.dwBufferBytes =
          0;                                 // Must be zero for primary buffer
      ...
      if (SUCCEEDED(
        ...
        if (SUCCEEDED(direct_sound->CreateSoundBuffer(
                &pri_buffer_descriptipn, &primary_sound_buffer, NULL))) {
          HRESULT error = primary_sound_buffer->SetFormat(&pri_wave_format);
          if (SUCCEEDED(error)) {
            OutputDebugStringA(
                "Primary buffer formatw as set. Direct sound initialized.");
          } else {
            // TODO: Failed format
          }
        } else {
      ...
      sec_buffer_descriptipn.dwBufferBytes =
          buffer_size;                       // Must be zero for primary buffer
      ...
      HRESULT error = direct_sound->CreateSoundBuffer(
          &sec_buffer_descriptipn, &global_secondary_buffer, 0);
      if (SUCCEEDED(error)) {
...
static void win32_fillSoundBuffer(Win32_Sound_Output* sound_output,
                                  DWORD byte_to_lock, DWORD bytes_to_writes,
  ...
  if (SUCCEEDED(global_secondary_buffer->Lock(byte_to_lock, bytes_to_writes,
    ...
    DWORD region_1_sample_count =
        region_1_size / sound_output->bytes_per_sample;
    i16* dest_sample = (i16*)region_1;
    i16* source_sample = (i16*)source_buffer->samples;
    for (DWORD sample_index = 0; sample_index < region_1_sample_count;
         ++sample_index) {
    ...
    DWORD region_2_sample_count =
        region_2_size / sound_output->bytes_per_sample;
    dest_sample = (i16*)region_2;
    for (DWORD sample_index = 0; sample_index < region_2_sample_count;
         ++sample_index) {
      *dest_sample++ = *source_sample++;
      *dest_sample++ = *source_sample++;
    ...
    global_secondary_buffer->Unlock(region_1, region_1_size, region_2,
                                    region_2_size);
  }
}
...
static void win32_clearBuffer(Win32_Sound_Output* sound_output) {
  ...
  if (SUCCEEDED(global_secondary_buffer->Lock(
          0, sound_output->secondary_buffer_size, &region_1, &region_1_size,
          &region_2, &region_2_size, 0))) {
...
DEBUG_PLATFORM_READ_ENTIRE_FILE(debug_platformReadEntireFile) {
  ...
  if (file_handle != INVALID_HANDLE_VALUE) {
    ...
    if (GetFileSizeEx(file_handle, &file_size)) {
      ...
      if (result.contents) {
        DWORD bytes_read;
        if (ReadFile(file_handle, result.contents, file_size_32, &bytes_read,
                     0) &&
...
static void render_gradient(Game_Offscreen_Buffer* buffer, int x_offset) {
  int num_of_pixels =
      (buffer->height * buffer->width) / buffer->bytes_per_pixel;
...
#if 1
static void win32_debugDrawVertical(Game_Offscreen_Buffer* Backbuffer, int X,
...
inline void win32_drawSoundBufferMarker(Game_Offscreen_Buffer* Backbuffer,
...
internal void win32_debugSyncDisplay(Game_Offscreen_Buffer* Backbuffer,
                                     int MarkerCount,
  ...
  real32 C = (real32)(Backbuffer->width - 2 * PadX) /
             (real32)SoundOutput->secondary_buffer_size;
  for (int MarkerIndex = 0; MarkerIndex < MarkerCount; ++MarkerIndex) {
    Win32_Debug_Time_Marker* ThisMarker = &Markers[MarkerIndex];
    Assert(ThisMarker->output_play_cursor < SoundOutput->secondary_buffer_size);
    Assert(ThisMarker->output_write_cursor <
           SoundOutput->secondary_buffer_size);
    Assert(ThisMarker->output_location < SoundOutput->secondary_buffer_size);
    Assert(ThisMarker->output_byte_count < SoundOutput->secondary_buffer_size);
    Assert(ThisMarker->flipe_play_cursor < SoundOutput->secondary_buffer_size);
    Assert(ThisMarker->flip_write_cursor < SoundOutput->secondary_buffer_size);
    ...
    if (MarkerIndex == CurrentMarkerIndex) {
      ...
      win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                  ThisMarker->output_play_cursor, PlayColor);
      win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                  ThisMarker->output_write_cursor, WriteColor);

      Top += LineHeight + PadY;
      Bottom += LineHeight + PadY;

      win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                  ThisMarker->output_location, PlayColor);
      win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                  ThisMarker->output_location +
                                      ThisMarker->output_byte_count,
                                  WriteColor);

      Top += LineHeight + PadY;
      Bottom += LineHeight + PadY;

      win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, FirstTop,
                                  Bottom, ThisMarker->expected_flip_play_cursor,
                                  ExpectedFlipColor);
    }

    win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                ThisMarker->flipe_play_cursor, PlayColor);
    win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                ThisMarker->flipe_play_cursor +
                                    480 * SoundOutput->bytes_per_sample,
                                PlayWindowColor);
    win32_drawSoundBufferMarker(Backbuffer, SoundOutput, C, PadX, Top, Bottom,
                                ThisMarker->flip_write_cursor, WriteColor);
  }
}

#endif

static void DEBUG_output_sound(Game_State* game_state,
...
//************************* DISPLAY CODE
//************************************************************
static void win32_resizeDIBSection(Win32_Offscreen_Buffer* buffer, int width,
                                   int height) {
  if (buffer->memory) {
  ...
  buffer->info.bmiHeader.biClrUsed =
      0; // Not needed, this gets us the maximum amount of colors.
...
static void win32_displayBufferInWindow(Win32_Offscreen_Buffer* buffer,
                                        HDC device_context, int window_width,
                                        int window_height) {
  // PatBlt(device_context, 0, 0, window_width, window_height, BLACKNESS);
  int offset_x = 10;
  int offset_y = 10;
  StretchDIBits(device_context, offset_x, offset_y, buffer->width,
...
static void win32_getInputFileLocation(Win32_State* state, bool32 input_stream,
                                       int slot_index, int dest_count,
                                       char* dest) {
  char temp[64];
  // TODO CHANGED for CLANG
#if CLANG_COMPILE
  wsprintf((LPWSTR)temp, (LPWSTR) "loop_edit_&d_%d.hmi", slot_index,
           input_stream ? "input" : "state");
#else
  wsprintf(temp, "loop_edit_&d_%d.hmi", slot_index,
           input_stream ? "input" : "state");
#endif
  win32_buildEXEPathFileName(state, temp, dest_count, dest);
}

static Win32_Replay_Buffer* win32_getReplayBuffer(Win32_State* state,
                                                  int unsigned index) {
...
static void win32_beginRecordingInput(Win32_State* state,
                                      int input_recording_index) {
  Win32_Replay_Buffer* replay_buffer =
      win32_getReplayBuffer(state, input_recording_index);
  if (replay_buffer->memory_block) {
    state->input_recording_index = input_recording_index;
    char filename[WIN32_STATE_FILE_NAME_COUNT];
    win32_getInputFileLocation(state, true, input_recording_index,
...
#endif
    CopyMemory(replay_buffer->memory_block, state->game_memory_block,
               state->total_size);
  }
}
...
static void win32_beginInputPlayBack(Win32_State* state,
                                     int input_playing_index) {
  Win32_Replay_Buffer* replay_buffer =
      win32_getReplayBuffer(state, input_playing_index);
  if (replay_buffer->memory_block) {
    state->input_playing_index = input_playing_index;

    char filename[WIN32_STATE_FILE_NAME_COUNT];
    win32_getInputFileLocation(state, true, input_playing_index,
                               sizeof(filename), filename);
    state->playback_handle =
        ...
        SetFilePointerEx(state->recording_handle, file_position, 0, FILE_BEGIN);
#endif
    CopyMemory(state->game_memory_block, replay_buffer->memory_block,
               state->total_size);
  }
}
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine,
  ...
  win32_resizeDIBSection(&global_back_buffer, INITIAL_WINDOW_X,
                         INITIAL_WINDOW_Y);
  ...
  HWND window =
      CreateWindowExA(0,                          // Optional window styles.
...
//kk
        ...
        _snprintf_s(text_buffer, sizeof(text_buffer), "PC:%u WC:%u UW:%u\n", play_cursor, write_cursor, unwrapped_write);
  ...
  win32_state.game_memory_block =
      VirtualAlloc(base_address, (size_t)win32_state.total_size,
                   MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  ...
  for (int replay_index = 0;
       replay_index < ArrayCount(win32_state.replay_buffers); ++replay_index) {
  }

  if (sound_samples && game_memory.permanent_storage &&
      game_memory.transient_storage) {
...
#if CLANG_COMPILE
    Win32_Game_Code game = win32_loadGameCode(source_game_code_dll_full_path,
                                              temp_game_code_dll_full_path);
#else
    Win32_Game_Code game = win32_loadGameCode(source_game_code_dll_full_path,
                                              temp_game_code_dll_full_path);
    ...
    while (RUNNING) {
      ...
      FILETIME new_dll_write_time =
          win32_getLastWriteTime(source_game_code_dll_full_path);
      if (CompareFileTime(&new_dll_write_time, &game.dll_last_write_time) !=
          0) {
        Win32_unloadGameCode(&game);
        game = win32_loadGameCode(source_game_code_dll_full_path,
                                  temp_game_code_dll_full_path);
        load_counter = 0;
      }
      ...
      *new_keyboard = {0};
      ...
      for (int button_index = 0;
           button_index < ArrayCount(new_keyboard->buttons); button_index++) {
      ...
      */
      ...
      for (DWORD controller_index = 0; controller_index < XUSER_MAX_COUNT;
        ...
        // +1, because the input[0] is for the keybaord and 1-4 is for
        // controllers.
        ...
        if (dwResult == ERROR_SUCCESS) {
          ...
          new_controller->left_stick_average_x = win32_process_x_input_stick(
              x_input_controller_state.Gamepad.sThumbLX,
              XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
          new_controller->left_stick_average_y = win32_process_x_input_stick(
              x_input_controller_state.Gamepad.sThumbLY,
              XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
          new_controller->right_stick_average_x = win32_process_x_input_stick(
              x_input_controller_state.Gamepad.sThumbRX,
              XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
          new_controller->right_stick_average_y = win32_process_x_input_stick(
              x_input_controller_state.Gamepad.sThumbRY,
              XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);

          // Buttons
          win32_process_x_input_button(
              x_input_controller_state.Gamepad.wButtons,
              &old_controller->action_down, XINPUT_GAMEPAD_A,
              &new_controller->action_down);
          win32_process_x_input_button(
              x_input_controller_state.Gamepad.wButtons,
              &old_controller->action_left, XINPUT_GAMEPAD_B,
              &new_controller->action_left);
          win32_process_x_input_button(
              x_input_controller_state.Gamepad.wButtons,
              &old_controller->action_right, XINPUT_GAMEPAD_X,
          ...
          win32_process_x_input_button(
              x_input_controller_state.Gamepad.wButtons, &old_controller->start,
              XINPUT_GAMEPAD_START, &new_controller->start);
          win32_process_x_input_button(
              x_input_controller_state.Gamepad.wButtons, &old_controller->back,
      ...
      real32 from_begin_to_audio_seconds =
          win32_getSecondsElapsed(flip_wall_clock, audio_wall_clock);

      DWORD play_cursor;
      DWORD write_cursor;
      if (global_secondary_buffer->GetCurrentPosition(&play_cursor,
                                                      &write_cursor) == DS_OK) {
        /* NOTE(casey):
        ...
        */
        ...
        DWORD expected_sound_bytes_per_frame =
            (int)((real32)sound_output.samples_per_second *
                  sound_output.bytes_per_sample) /
            game_update_hz;
        real32 seconds_left_until_flip =
        ...
        bool32 audio_card_is_low_latency =
        ...
        DWORD UnwrappedWriteCursor = write_cursor;
        if (UnwrappedWriteCursor < play_cursor) {
          UnwrappedWriteCursor += sound_output.secondary_buffer_size;
        }
        audio_latency_bytes = UnwrappedWriteCursor - play_cursor;
        audio_latency_seconds = (((real32)audio_latency_bytes /
                                  (real32)sound_output.bytes_per_sample) /
      ...
      real32 work_seconds_elapsed =
          win32_getSecondsElapsed(last_counter, work_counter);

      // Sleep
      real32 seconds_elapsed_for_frame = work_seconds_elapsed;
      if (seconds_elapsed_for_frame < target_seconds_per_frame) {
        if (sleep_is_grannular) {
          sleep_ms = (DWORD)(1000.0f * (target_seconds_per_frame -
                                        seconds_elapsed_for_frame));
          if (sleep_ms > 0) {
            Sleep(sleep_ms);
          }
        }
        real32 test_seconds_elapsed_for_frame =
            win32_getSecondsElapsed(last_counter, win32_getWallClock());
        ...
        // TODO: missed frame rate loggings!!
      }

      LARGE_INTEGER end_counter = win32_getWallClock();
      real32 ms_per_frame =
          1000.0f * win32_getSecondsElapsed(last_counter, end_counter);
      ...
      // NOTE(casey): This is debug code
      {
        ...
        if (global_secondary_buffer->GetCurrentPosition(
          ...
          Win32_Debug_Time_Marker* marker =
              &debug_time_markers[debug_time_marker_index];
          marker->flipe_play_cursor = play_cursor;
      ...
      // real64 fps = perf_count_frequency_result / (end_counter.QuadPart -
      // last_counter.QuadPart);
      real64 fps = (1 / ms_per_frame);
      real64 mega_cycles_per_frame =
          ((real64)cycles_elapsed / (1000.0f * 1000.0f));
    ...
    case WM_KEYUP: {
      ...
      if (was_down != is_down) {
        if (VKCode == 'W') {
          ...
          win32_process_keyboard_message(&keyboard_controller->left_shoulder,
                                         is_down);
        } else if (VKCode == 'E') {
          win32_process_keyboard_message(&keyboard_controller->right_shoulder,
                                         is_down);
        } else if (VKCode == VK_UP) {
          win32_process_keyboard_message(&keyboard_controller->action_up,
                                         is_down);
        } else if (VKCode == VK_LEFT) {
          win32_process_keyboard_message(&keyboard_controller->action_left,
                                         is_down);
        } else if (VKCode == VK_DOWN) {
          win32_process_keyboard_message(&keyboard_controller->action_down,
                                         is_down);
        } else if (VKCode == VK_RIGHT) {
          win32_process_keyboard_message(&keyboard_controller->action_right,
                                         is_down);
...
static LRESULT CALLBACK WindowProc(HWND window, UINT uMsg, WPARAM wParam,
                                   LPARAM lParam) {
  LRESULT result = 0;
  ...
  case WM_SIZE: {
    ...
    render_buffer.pixels = (u32*)VirtualAlloc(0, sizeof(u32) *
    render_buffer.num_of_pixels, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);

    render_buffer.bitmap_info.bmiHeader.biSize =
    sizeof(render_buffer.bitmap_info.bmiHeader);
    ...
    render_buffer.bitmap_info.bmiHeader.biClrUsed = 0; // Not needed, this gets
    us the maximum amount of colors.
    render_buffer.bitmap_info.bmiHeader.biClrImportant = 0; // All colors are
    important.
    */
  } break;
  case WM_PAINT: {
    ...
    Win32_Window_Dimension dimension = win32_getWindowDimension(window);
    win32_displayBufferInWindow(&global_back_buffer, hdc, dimension.width,
                                dimension.height);
    EndPaint(window, &ps);
  }
  default: {
