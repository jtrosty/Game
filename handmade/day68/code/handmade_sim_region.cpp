...
internal sim_entity_hash* GetHashFromStorageIndex(sim_region* SimRegion,
                                                  uint32 StorageIndex) {
  Assert(StorageIndex);
...
inline sim_entity* GetEntityByStorageIndex(sim_region* SimRegion,
                                           uint32 StorageIndex) {
...
internal sim_entity* AddEntity(game_state* GameState, sim_region* SimRegion,
                               ...
                               v2* SimP);
inline void LoadEntityReference(game_state* GameState, sim_region* SimRegion,
...
internal sim_region* BeginSim(memory_arena* SimArena, game_state* GameState,
                              ...
                              rectangle2 Bounds) {
  // TODO(casey): If entities were stored in the world, we wouldn't need the
  // game state here!

  sim_region* SimRegion = PushStruct(SimArena, sim_region);
  ZeroStruct(SimRegion->Hash);

  // TODO(casey): IMPORTANT(casey): Calculate this eventually from the maximum
  ...
  SimRegion->Entities =
      PushArray(SimArena, SimRegion->MaxEntityCount, sim_entity);
  ...
  for (int32 ChunkY = MinChunkP.ChunkY; ChunkY <= MaxChunkP.ChunkY; ++ChunkY) {
    for (int32 ChunkX = MinChunkP.ChunkX; ChunkX <= MaxChunkP.ChunkX;
         ++ChunkX) {
      world_chunk* Chunk =
          GetWorldChunk(World, ChunkX, ChunkY, SimRegion->Origin.ChunkZ);
      if (Chunk) {
        for (world_entity_block* Block = &Chunk->FirstBlock; Block;
             Block = Block->Next) {
          for (uint32 EntityIndexIndex = 0;
               EntityIndexIndex < Block->EntityCount; ++EntityIndexIndex) {
            ...
            if (!IsSet(&Low->Sim, EntityFlag_Nonspatial)) {
              ...
              if (IsInRectangle(SimRegion->Bounds, SimSpaceP)) {
                AddEntity(GameState, SimRegion, LowEntityIndex, Low,
                          &SimSpaceP);
              }
            }
          }
...
internal void EndSim(sim_region* Region, game_state* GameState) {
  // TODO(casey): Maybe don't take a game state here, low entities should be
  // stored in the world??
...
internal bool32 TestWall(real32 WallX, real32 RelX, real32 RelY,
                         real32 PlayerDeltaX, real32 PlayerDeltaY, real32* tMin,
