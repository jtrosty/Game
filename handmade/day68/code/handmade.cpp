...
internal void DrawRectangle(game_offscreen_buffer* Buffer, v2 vMin, v2 vMax,
...
internal void DrawBitmap(game_offscreen_buffer* Buffer, loaded_bitmap* Bitmap,
  ...
  uint8* DestRow = ((uint8*)Buffer->Memory + MinX * Buffer->BytesPerPixel +
                    MinY * Buffer->Pitch);
  for (int Y = MinY; Y < MaxY; ++Y) {
...
internal add_low_entity_result AddLowEntity(game_state* GameState,
                                            entity_type Type,
                                            world_position P) {
  ...
  *EntityLow = {};
  ...
  ChangeEntityLocation(&GameState->WorldArena, GameState->World, EntityIndex,
                       EntityLow, P);
...
internal add_low_entity_result AddWall(game_state* GameState, uint32 AbsTileX,
...
internal void InitHitPoints(low_entity* EntityLow, uint32 HitPointCount) {
  ...
  for (uint32 HitPointIndex = 0; HitPointIndex < EntityLow->Sim.HitPointMax;
       ++HitPointIndex) {
...
internal add_low_entity_result AddSword(game_state* GameState) {
  add_low_entity_result Entity =
      AddLowEntity(GameState, EntityType_Sword, NullPosition());
...
internal add_low_entity_result AddMonstar(game_state* GameState,
...
internal add_low_entity_result AddFamiliar(game_state* GameState,
                                           uint32 AbsTileX, uint32 AbsTileY,
                                           uint32 AbsTileZ) {
  world_position P = ChunkPositionFromTilePosition(GameState->World, AbsTileX,
                                                   AbsTileY, AbsTileZ);
  add_low_entity_result Entity =
      AddLowEntity(GameState, EntityType_Familiar, P);
...
inline void PushPiece(entity_visible_piece_group* Group, loaded_bitmap* Bitmap,
                      v2 Offset, real32 OffsetZ, v2 Align, v2 Dim, v4 Color,
                      real32 EntityZC) {
  Assert(Group->PieceCount < ArrayCount(Group->Pieces));
  entity_visible_piece* Piece = Group->Pieces + Group->PieceCount++;
  Piece->Bitmap = Bitmap;
  Piece->Offset =
      Group->GameState->MetersToPixels * V2(Offset.X, -Offset.Y) - Align;
...
inline void PushRect(entity_visible_piece_group* Group, v2 Offset,
                     real32 OffsetZ, v2 Dim, v4 Color, real32 EntityZC = 1.0f) {
  PushPiece(Group, 0, Offset, OffsetZ, V2(0, 0), Dim, Color, EntityZC);
}

internal void DrawHitpoints(sim_entity* Entity,
  ...
  if (Entity->HitPointMax >= 1) {
    ...
    for (uint32 HealthIndex = 0; HealthIndex < Entity->HitPointMax;
         ++HealthIndex) {
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender) {
  ...
  if (!Memory->IsInitialized) {
    ...
    GameState->MetersToPixels =
        (real32)TileSideInPixels / (real32)World->TileSideInMeters;
  ...
  InitializeArena(&SimArena, Memory->TransientStorageSize,
  ...
  //
#if 1
  DrawRectangle(Buffer, V2(0.0f, 0.0f),
                V2((real32)Buffer->Width, (real32)Buffer->Height), 0.5f, 0.5f,
                0.5f);
#else
  ...
  for (uint32 EntityIndex = 0; EntityIndex < SimRegion->EntityCount;
    ...
    if (Entity->Updatable) {
      ...
      case EntityType_Hero: {
        // TODO(casey): Now that we have some real usage examples, let's
        ...
        // TODO(casey): Z!!!
        PushBitmap(&PieceGroup, &GameState->Shadow, V2(0, 0), 0,
                   HeroBitmaps->Align, ShadowAlpha, 0.0f);
        PushBitmap(&PieceGroup, &HeroBitmaps->Torso, V2(0, 0), 0,
                   HeroBitmaps->Align);
        PushBitmap(&PieceGroup, &HeroBitmaps->Cape, V2(0, 0), 0,
                   HeroBitmaps->Align);
        PushBitmap(&PieceGroup, &HeroBitmaps->Head, V2(0, 0), 0,
                   HeroBitmaps->Align);
      ...
      case EntityType_Familiar: {
        ...
        real32 ClosestHeroDSq =
            Square(10.0f); // NOTE(casey): Ten meter maximum search!
        ...
        PushBitmap(&PieceGroup, &GameState->Shadow, V2(0, 0), 0,
                   HeroBitmaps->Align, (0.5f * ShadowAlpha) + 0.2f * BobSin,
                   0.0f);
        PushBitmap(&PieceGroup, &HeroBitmaps->Head, V2(0, 0), 0.25f * BobSin,
                   HeroBitmaps->Align);
      } break;
  ...
  world_difference Diff =
      Subtract(SimRegion->World, &WorldOrigin, &SimRegion->Origin);
