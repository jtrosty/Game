...
inline bool32 IsCanonical(world* World, v2 Offset) {
  bool32 Result =
      (IsCanonical(World, Offset.X) && IsCanonical(World, Offset.Y));
...
inline bool32 AreInSameChunk(world* World, world_position* A,
  ...
  bool32 Result = ((A->ChunkX == B->ChunkX) && (A->ChunkY == B->ChunkY) &&
                   (A->ChunkZ == B->ChunkZ));
...
internal void InitializeWorld(world* World, real32 TileSideInMeters) {
  ...
  for (uint32 ChunkIndex = 0; ChunkIndex < ArrayCount(World->ChunkHash);
       ++ChunkIndex) {
...
inline world_position MapIntoChunkSpace(world* World, world_position BasePos,
                                        v2 Offset) {
...
inline world_position ChunkPositionFromTilePosition(world* World,
  ...
  // TODO(casey): Think this through on the real stream and actually work out
  // the math.
  if (AbsTileX < 0) {
    --Result.ChunkX;
  }
...
inline world_difference Subtract(world* World, world_position* A,
                                 world_position* B) {
...
inline world_position CenteredChunkPoint(uint32 ChunkX, uint32 ChunkY,
                                         uint32 ChunkZ) {
  world_position Result = {};
...
inline void ChangeEntityLocationRaw(memory_arena* Arena, world* World,
                                    uint32 LowEntityIndex, world_position* OldP,
                                    world_position* NewP) {
  ...
  if (OldP && NewP && AreInSameChunk(World, OldP, NewP)) {
    ...
    if (OldP) {
      ...
      if (Chunk) {
        ...
        for (world_entity_block* Block = FirstBlock; Block && NotFound;
             Block = Block->Next) {
          for (uint32 Index = 0; (Index < Block->EntityCount) && NotFound;
               ++Index) {
...
internal void ChangeEntityLocation(memory_arena* Arena, world* World,
