...
void Render::test_drawGameBoard(GameData bkgdRec[], int width, int height, int row, int col, int offset) { 
    for (int i = 0; i < 40; i++) {
        //test_drawGameBoard(&(bkgdRec[0]));
        DrawRectangle(bkgdRec[i].x, bkgdRec[i].y, bkgdRec[i].width, bkgdRec[i].height, bkgdRec[i].color);
    }
    for (int i = 0; i <= row; i++) {
        DrawLine(i * width + offset, offset, i * width + offset, offset + row * height, BLACK);
    }
    for (int i = 0; i <= col; i++) {
        DrawLine( offset, offset + (i * height),  offset + (col * width), offset + (i * height), BLACK);
    }
}